...
app.post('/conference-bookings', authenticateToken, isAdmin, (req, res) => {
  const { facility_id, name, date, start_time, end_time, deposit = 0.0, status } = req.body;
  ...
  `, [facility_id, date, end_time, start_time], (err, clash) => {
    ...
    db.get("SELECT hourly_rate, daily_rate FROM conferences WHERE id = ?", [facility_id], (err, facility) => {
      ...
      db.run(
        "INSERT INTO conference_bookings (facility_id, name, date, start_time, end_time, status, deposit, total_price) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
        [facility_id, name, date, start_time, end_time, status || 'reserved', parseFloat(deposit), total_price], function(err) {
