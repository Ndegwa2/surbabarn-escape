...
const TAX_RATE = 0.16; // 16% VAT, hardcoded for simplicity
...
const DBHelpers = {
  ...
  getAll: async function(tableName, orderBy = 'id') {
    return new Promise((resolve, reject) => {
      db.all(`SELECT * FROM ${tableName} ORDER BY ${orderBy}`, (err, rows) => {
        if (err) reject(err);
        else resolve(rows || []);
      });
    });
  ...
  insert: async function(tableName, data) {
    ...
    return new Promise((resolve, reject) => {
      db.run(`INSERT INTO ${tableName} (${columns.join(', ')}) VALUES (${placeholders})`,
        values, function(err) {
          if (err) reject(err);
  ...
  update: async function(tableName, id, data) {
    ...
    return new Promise((resolve, reject) => {
      db.run(`UPDATE ${tableName} SET ${setClause} WHERE id = ?`, [...values, id], function(err) {
        if (err) reject(err);
  ...
  findByField: async function(tableName, field, value) {
    return new Promise((resolve, reject) => {
      db.all(`SELECT * FROM ${tableName} WHERE ${field} = ?`, [value], (err, rows) => {
        if (err) reject(err);
        else resolve(rows || []);
      });
    });
  ...
  count: async function(tableName, whereClause = '', params = []) {
    return new Promise((resolve, reject) => {
...
async function initializeDatabase() {
  ...
  // Create housekeeping_tasks table if not exists
  await run(`
  ...
  // Create supply_requests table if not exists
  await run(`
  ...
  await run(`
    CREATE TABLE IF NOT EXISTS invoices (
      ...
      FOREIGN KEY (conference_booking_id) REFERENCES conference_bookings (id) ON DELETE SET NULL
    )
  `);
...
}  // Closing brace for initializeDatabase function
...
const ValidationHelpers = {
  ...
  validateStock: (current, total) => {
    if (current > total) {
      return 'Current stock cannot exceed total stock';
    }
    return null;
  }
};
    ...
    bcrypt.hash(password, 10, (err, hash) => {
      ...
      db.run("INSERT INTO users (username, password_hash, role) VALUES (?, ?, ?)",
        [username, hash, role], function(err) {
          ...
          logActivity('create', 'user', this.lastID, `Created user ${username} with role ${role}`, 'admin');
...
app.post('/checkout/:id', authenticateToken, (req, res) => {
  ...
  db.get("SELECT b.id as booking_id, b.room_id, b.status FROM bookings b JOIN guests g ON b.guest_id = g.id WHERE g.id = ? AND b.status = 'checked_in'", [guestId], (err, booking) => {
    ...
    db.run("UPDATE rooms SET status = 'available', housekeeping_state = 'dirty' WHERE id = ?", [booking.room_id]);
    db.run("UPDATE guests SET status = 'Checked Out' WHERE id = ?", [guestId]);
    
    // Create a housekeeping task for this room
    db.run(
      "INSERT INTO housekeeping_tasks (room_id, status) VALUES (?, 'assigned')",
      [booking.room_id],
      (err) => {
        if (err) {
        ...
        res.status(200).send('Guest checked out successfully. Room marked for cleaning.');
      }
    );
  });
});
...
app.get('/bookings', authenticateToken, (req, res) => {
  ...
  `, (err, results) => {
    if (err) {
      ...
      return res.status(500).send('Error fetching bookings: ' + err.message);
    }
    res.json(results);
  });
});
...
// Create invoice for room booking (protected)
app.post('/invoices/room/:bookingId', authenticateToken, (req, res) => {
  ...
  `, [bookingId], (err, booking) => {
    ...
    const roomTotal = booking.base_rate * nights;

    // Fetch allocated accessories for this booking
    db.all(`
      SELECT aa.quantity, aa.status, a.name as accessory_name, a.price_per_unit
      FROM accessory_allocations aa
      JOIN accessories a ON aa.accessory_id = a.id
      WHERE aa.booking_id = ? AND aa.status IN ('allocated', 'returned')
    `, [bookingId], (err, allocations) => {
      if (err) {
      ...
      for (const alloc of allocations) {
        const lineTotal = alloc.quantity * alloc.price_per_unit;
        accessoryTotal += lineTotal;
        accessoryItems.push({
          description: `${alloc.accessory_name} (${alloc.status})`,
          qty: alloc.quantity,
          unit_price: alloc.price_per_unit,
          line_total: lineTotal
        });
      }

      const subtotal = roomTotal + accessoryTotal;
      const tax = subtotal * TAX_RATE;
      const totalAmount = subtotal + tax;

      // Insert invoice
      db.run(
        "INSERT INTO invoices (booking_id, issue_date, due_date, total_amount, status) VALUES (?, CURRENT_DATE, date(CURRENT_DATE, '+30 days'), ?, 'draft')",
        ...
        function(err) {
          ...
          db.run(
            ...
            [invoiceId, `Room stay: ${booking.room_name} (${nights} nights)`, nights, booking.base_rate, roomTotal],
            function(err) {
              if (err) return res.status(500).json({ error: 'Error adding room item' });
              ...
              if (accessoryItems.length > 0) {
                accessoryItems.forEach(item => {
                  db.run(
                    "INSERT INTO invoice_items (invoice_id, description, qty, unit_price, line_total) VALUES (?, ?, ?, ?, ?)",
                    [invoiceId, item.description, item.qty, item.unit_price, item.line_total],
                    function(err) {
                      if (err) console.error('Error adding accessory item:', err);
                    }
                  );
                });
              }
              ...
              // Log activity
              logActivity('create', 'invoice', invoiceId, `Created for room booking ${bookingId}, guest: ${booking.guest_name}`, req.user.role);
              ...
              res.json({
                ...
                items: [
                  { description: `Room stay: ${booking.room_name} (${nights} nights)`, qty: nights, unit_price: booking.base_rate, line_total: roomTotal },
                  ...accessoryItems
                ],
                subtotal,
                tax,
...
// Supply request endpoints (staff and admin)
app.post('/supply-requests', authenticateToken, async (req, res) => {
  const { item_name, quantity, reason } = req.body;
  const maid_id = req.user.id;
  
  if (!item_name || !quantity) {
    return res.status(400).json({ error: 'Item name and quantity required' });
  }
  
  try {
    const result = await run(
      "INSERT INTO supply_requests (maid_id, item_name, quantity, reason, status) VALUES (?, ?, ?, ?, 'pending')",
      [maid_id, item_name, quantity, reason || '']
    );
    
    // Log activity
    const description = `Requested ${quantity} ${item_name}`;
    logActivity('create', 'supply_request', result.lastID, description, req.user.role);
    
    res.json({ id: result.lastID, message: 'Supply request submitted successfully' });
  } catch (err) {
...
app.get('/supply-requests', authenticateToken, (req, res) => {
  const query = req.user.role === 'admin'
    ? "SELECT sr.*, u.username as maid_name FROM supply_requests sr JOIN users u ON sr.maid_id = u.id ORDER BY sr.request_date DESC"
    : "SELECT * FROM supply_requests WHERE maid_id = ? ORDER BY request_date DESC";
    
  const params = req.user.role === 'admin' ? [] : [req.user.id];
  
  db.all(query, params, (err, requests) => {
    if (err) return res.status(500).json({ error: 'Failed to fetch supply requests' });
    res.json(requests);
  });
});

app.put('/supply-requests/:id', authenticateToken, isAdmin, async (req, res) => {
  ...
  try {
    const result = await run(
      "UPDATE supply_requests SET status = ? WHERE id = ?",
      [status, id]
    );
    ...
    // Log activity
    logActivity('update', 'supply_request', id, `Status changed to ${status}`, req.user.role);
...
app.post('/housekeeping-tasks', authenticateToken, isAdmin, async (req, res) => {
  const { room_id, maid_id, notes } = req.body;
  ...
  try {
    const result = await run(
      "INSERT INTO housekeeping_tasks (room_id, maid_id, assigned_by, status, notes) VALUES (?, ?, ?, 'assigned', ?)",
      [room_id, maid_id || null, req.user.id, notes || '']
    );
    
    // Update room housekeeping state
    await run("UPDATE rooms SET housekeeping_state = 'dirty' WHERE id = ?", [room_id]);
    
    // Log activity
    const description = maid_id ? `Task assigned to maid ${maid_id}` : 'Task created (unassigned)';
    logActivity('assign', 'housekeeping_task', result.lastID, description, req.user.role);
    
    res.json({ id: result.lastID, message: 'Housekeeping task created successfully' });
  } catch (err) {
    console.error('Error creating housekeeping task:', err);
...
app.get('/housekeeping-tasks', authenticateToken, (req, res) => {
  const query = req.user.role === 'admin'
    ? `SELECT ht.*, r.name as room_name,
              COALESCE(m.username, 'Unassigned') as maid_name,
              u.username as assigned_by_name
       FROM housekeeping_tasks ht
       JOIN rooms r ON ht.room_id = r.id
       LEFT JOIN users m ON ht.maid_id = m.id
       LEFT JOIN users u ON ht.assigned_by = u.id
       ORDER BY ht.assigned_date DESC`
    : `SELECT ht.*, r.name as room_name
       FROM housekeeping_tasks ht
       JOIN rooms r ON ht.room_id = r.id
       WHERE ht.maid_id = ? AND ht.status != 'completed'
       ORDER BY ht.assigned_date DESC`;
    
  const params = req.user.role === 'admin' ? [] : [req.user.id];
  
  db.all(query, params, (err, tasks) => {
    if (err) return res.status(500).json({ error: 'Failed to fetch housekeeping tasks' });
    res.json(tasks);
  });
});

app.put('/housekeeping-tasks/:id', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    // For maids, only allow updating their own tasks
    if (req.user.role !== 'admin') {
      query += " AND maid_id = ?";
    ...
    if (result.changes === 0) {
      return res.status(404).json({ error: 'Housekeeping task not found or not authorized' });
    }
    
    // If task is completed, update room housekeeping state
    if (status === 'completed') {
      // Get room ID for this task
      db.get("SELECT room_id FROM housekeeping_tasks WHERE id = ?", [id], async (err, task) => {
        if (!err && task) {
          await run("UPDATE rooms SET housekeeping_state = 'clean' WHERE id = ?", [task.room_id]);
        }
      });
    }
    
    // Log activity
    logActivity('update', 'housekeeping_task', id, `Status changed to ${status}`, req.user.role);
...
}); // Close the app.put('/housekeeping-tasks/:id') function
...
app.put('/users/:id/reset-password', authenticateToken, isAdmin, async (req, res) => {
  const { id } = req.params;
  ...
  try {
    ...
    db.run("UPDATE users SET password_hash = ? WHERE id = ?", [hashedPassword, id], function(err) {
      ...
      // Log the activity
      logActivity('reset_password', 'user', id, 'Password reset by admin', req.user.role);
