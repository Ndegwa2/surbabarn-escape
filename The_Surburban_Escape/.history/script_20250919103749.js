document.addEventListener('DOMContentLoaded', function () {
  ...
  async function loadRoomsKanban() {
    ...
    const cleanRooms = document.getElementById('cleanRooms');
    ...
    rooms.forEach(room => {
      ...
      item.innerHTML = `
        <div><strong>${room.name}</strong> (${room.type}) - $${room.base_rate}</div>
        <div>Status: ${room.status} | Housekeeping: ${housekeepingState}</div>
        <div class="quick-actions">
          <button onclick="updateHousekeepingState(${room.id}, 'dirty')">Mark Dirty</button>
          <button onclick="updateHousekeepingState(${room.id}, 'clean')">Mark Clean</button>
          <button onclick="updateHousekeepingState(${room.id}, 'maintenance')">Mark Maintenance</button>
        </div>
      `;
