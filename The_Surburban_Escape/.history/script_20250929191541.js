...
const ModalManager = {
  // Configure modal with close functionality
  configureModal: function(modalId, closeBtnSelector = '.close', displayStyle = 'none') {
    const modal = document.getElementById(modalId);
    if (!modal) return null;

    const closeBtn = document.querySelector(`#${modalId} ${closeBtnSelector}`);
    if (closeBtn) {
    ...
    return {
      show: () => modal.style.display = displayStyle === 'none' ? 'block' : 'flex',
      hide: () => modal.style.display = displayStyle,
      toggle: () => modal.style.display = modal.style.display === displayStyle ? (displayStyle === 'none' ? 'block' : 'flex') : displayStyle
    };
  },

  // Show modal by ID
  show: function(modalId, displayStyle = 'none') {
    const modal = document.getElementById(modalId);
    if (modal) {
      modal.style.display = displayStyle === 'none' ? 'block' : 'flex';
    }
  },
...
// Configure modals
const itemModal = ModalManager.configureModal('itemDetailsModal');
const usageModal = ModalManager.configureModal('addUsageModal', '#closeUsageModal');
const roomModal = ModalManager.configureModal('roomDetailsModal', '#closeRoomModal');
